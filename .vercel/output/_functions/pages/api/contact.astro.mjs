...
const resend = new Resend("re_72Sjwjdj_AwpLDNXae5xiuw2FX8r6K15r");
const POST = async ({ request }) => {
  try {
    const data = await request.json();
    const { firstName, lastName, email, phone, company, subject, message, consent } = data;
    ...
    if (!consent) {
      return new Response(JSON.stringify({ error: "Vous devez accepter la politique de confidentialité." }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #ffffff; padding: 40px; border-radius: 8px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 8px 0; color: #888; font-size: 13px; width: 140px;">Prénom</td>
              <td style="padding: 8px 0; color: #fff; font-size: 13px;">${firstName}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; color: #888; font-size: 13px;">Nom</td>
              <td style="padding: 8px 0; color: #fff; font-size: 13px;">${lastName}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; color: #888; font-size: 13px;">Email</td>
              <td style="padding: 8px 0; color: #C8960C; font-size: 13px;"><a href="mailto:${email}" style="color: #C8960C;">${email}</a></td>
            </tr>
          ...
          <p style="color: #555; font-size: 11px; text-align: center; margin: 0;">Ce message a été envoyé depuis le formulaire de contact de onenationagency.com</p>
        </div>
      `
    });
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #ffffff; padding: 40px; border-radius: 8px;">
          ...
          <p style="color: #555; font-size: 11px; text-align: center; margin: 0;">
            <a href="https://onenationagency.com" style="color: #C8960C;">onenationagency.com</a> — contact@onenationagency.com
          </p>
        </div>
      `
    });
    ...
    return new Response(JSON.stringify({ error: "Erreur lors de l'envoi. Veuillez réessayer." }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
  }
